function _rename_labels(arguments) {
  const { data, dimension, label, should_corrupt_labels = false } = arguments;
  const dataCube = new DataCube(data, "bands_name", "temporal_name");

  dataCube.addDimension(dimension, label, "other");

  if (should_corrupt_labels) {
    dataCube.getDimensionByName(dimension).labels = "corrupted_labels";
  }

  try {
    return rename_labels({ ...arguments, data: dataCube });
  } catch (e) {
    return e.message;
  }
}
