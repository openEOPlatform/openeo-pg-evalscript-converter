function _filter_bands(arguments) {
  const { data, shouldRemoveBandsDim = false } = arguments;
  const dataCube = new DataCube(data, "bands_name", "temporal_name", true);

  if (shouldRemoveBandsDim) {
    dataCube.removeDimension(dataCube.bands_dimension_name);
  }

  try {
    return filter_bands({ ...arguments, data: dataCube });
  } catch (e) {
    return e.message;
  }
}
