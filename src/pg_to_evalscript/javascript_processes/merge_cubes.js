function _merge_cubes(arguments) {
  const {
    cube1,
    cube2,
    should_use_overlap_resolver = false,
    overlap_resolver_code = "",
  } = arguments;
  const newCube1 = new DataCube(cube1, "bands_name", "temporal_name", true);
  const newCube2 = new DataCube(cube2, "bands_name", "temporal_name", true);

  let overlap_resolver;
  if (should_use_overlap_resolver) {
    overlap_resolver = eval(overlap_resolver_code);
  }
...
class DataCube {
  ...
  makeArrayFromSamples(samples) {
    if (Array.isArray(samples)) {
      ...
      for (let entry of samples) {
        if (
          this.getDimensionByName(this.bands_dimension_name).labels.length === 0
        ) {
  ...
  getBandIndices(bands) {
    return bands.map((b) =>
      this.getDimensionByName(this.bands_dimension_name).labels.indexOf(b)
    );
  }
  ...
  clone() {
    const copy = new DataCube(
      this._iter(this.data),
      this.bands_dimension_name,
      this.temporal_dimension_name
    );
  ...
  _iter(
    ...
    execute = (v) => v,
    removeDim = () => false,
    execute_on_result = (r) => r,
    coords = []
  ) {
    if (Array.isArray(arr)) {
      let result = [];
      for (let i = 0; i < arr.length; i++) {
        const val = this._iter(arr[i], execute, removeDim, execute_on_result, [
          ...coords,
          i,
        ]);
        if (val !== undefined) {
  ...
  _select(arr, coordArr) {
    ...
    return this._iter(
      ...
      (coords) =>
        coords.length >= coordArr.length || coordArr[coords.length] === null
          ? false
          : true
    );
  }
  ...
  _set(arr, vals, coordArr) {
    ...
    function coordInSlice(c1, sliceArr) {
      return (
        c1.length === sliceArr.length &&
        sliceArr.every((e, i) => e === null || e === c1[i])
      );
    }
    const exec_set = (a, coords) => {
      if (coordInSlice(coords, coordArr)) {
        ...
        if (Array.isArray(vals)) {
          valueToSet = this._select(
            vals,
            coordArr
              .map((c, i) => (c === null ? coords[i] : null))
              .filter((c) => c !== null)
          );
        } else {
          valueToSet = vals;
