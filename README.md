...
* **`process_graph`**: *dict*
    ...
    Process graph JSON as Python dict object.

* **`n_output_bands`**: *int, optional*. Default: `1`

    Number of output bands in the evalscript. This can be set if the value is known beforehand. See [docs](https://docs.sentinel-hub.com/api/latest/evalscript/v3/#input-object-properties).
...
* **`bands_dimension_name`**: *str, optional*. Default: `bands`
    ...
    Name of the default dimension of type `bands` of the datacube, as set in `load_collection` and referred to in the process graph.

* **`temporal_dimension_name`**: *str, optional*. Default: `t`

    Name of the default dimension of type `temporal` of the datacube, as set in `load_collection` and referred to in the process graph.
...
* **`evalscripts`**: *list of dicts*
    ...
    - `evalscript`: instance of and `Evalscript` object that generates a an evalscript for a valid subgraph from `load_collection` to the node with id `invalid_node_id`. 
...
1. Construct the process graph

Load a file with `json.load` or generate a process graph using [openEO Python client](https://github.com/Open-EO/openeo-python-client).
...
In this example, the entire process graph could be converted to an evalscript, so we only have one entry.
...
```python
# Get the decoding function for this evalsscript
